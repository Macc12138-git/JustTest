...
{
    public sealed class RangedEnemyWhiteboxMotionPresenter2D : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (!ready)
            {
                Debug.LogError($"{nameof(RangedEnemyWhiteboxMotionPresenter2D)} is missing an Inspector reference.", this);
                enabled = false;
                return;
            }
            ...
            bool grounded)
        {
            ...
            if (reactionReceiver.IsReacting)
            {
                ...
                float reactionDirection = Mathf.Sign(reactionReceiver.CurrentReaction.KnockbackVelocity.x);
            ...
            if (statusController.ActiveStatusCount > 0)
            {
                ...
                float wave = Mathf.Sin(Time.time * locomotionConfig.ControlledWobbleFrequency);
                return BuildBodyPose(
                    Vector2.zero,
            ...
            {
                CurrentMotionState = $"Ranged Attack {attackRunner.Phase}";
                return attackRunner.Phase == AttackPhase.Idle
                    ? EvaluatedMotionPose2D.Identity
                    : activeProfile.Evaluate(attackRunner.Phase, attackRunner.PhaseProgress);
            }
