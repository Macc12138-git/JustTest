...
{
    public sealed class EnemyWhiteboxMotionPresenter2D : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (!ready)
            {
                Debug.LogError($"{nameof(EnemyWhiteboxMotionPresenter2D)} is missing an Inspector reference.", this);
                enabled = false;
                return;
            }
