...
namespace JustTest.Game.Enemies
{
    ...
    public sealed class RangedEnemyMotor2D : MonoBehaviour
    {
        [SerializeField] private RangedEnemyConfig config;
        [SerializeField] private RangedEnemyGroundProbe2D groundProbe;
        ...
        private void Awake()
        {
            ready = config != null && config.IsValid && groundProbe != null && body != null;
            if (!ready)
            {
                Debug.LogError($"{nameof(RangedEnemyMotor2D)} is missing an Inspector reference.", this);
                enabled = false;
            }
        }
